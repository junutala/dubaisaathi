...
/**
 * Where a journey starts: the phone's fix when it gives one, the hotel's pin when it does not.
 * A traveller planning tomorrow from the hotel room is the common case, and a phone that has
...
export function useOrigin(hotel: SavedHotel | undefined): Origin {
  ...
  if (location.kind === 'here') return { kind: 'phone', at: location.at };
