...
-- the same database, with 0007 and 0009 applied:
--
...
--
-- Codes are eight characters with no separator since 0009; 11 is the case that holds that shape.
--
...
insert into coupons (code, kind, discount_percent, max_redemptions, batch) values
  ('SS7K3M2X', 'family', 100, 2, 'test batch'),
  ('OP4HXR9B', 'single', 50, 10, 'test operator');

\echo '== 1. a discount of 101 (must fail)'
insert into coupons (code, kind, discount_percent, max_redemptions, batch)
  values ('SSAAAAAA', 'family', 101, 1, 'bad');

\echo '== 2. a kind other than family or single (must fail)'
insert into coupons (code, kind, discount_percent, max_redemptions, batch)
  values ('SSBBBBBB', 'corporate', 10, 1, 'bad');

\echo '== 3. a code with an ambiguous character, or too short (must fail, twice)'
insert into coupons (code, kind, discount_percent, max_redemptions, batch)
  values ('SS0O1I2X', 'family', 10, 1, 'bad');
insert into coupons (code, kind, discount_percent, max_redemptions, batch)
  values ('SS7K3M2', 'family', 10, 1, 'bad');

\echo '== 4. a redemption of five slots (must fail)'
insert into coupon_redemptions (code, device_id, slots)
  values ('SS7K3M2X', '44444444-4444-4444-4444-444444444444', 5);

\echo '== 5. the same phone taking the same code twice (first succeeds, second must fail)'
insert into coupon_redemptions (code, device_id, slots)
  values ('SS7K3M2X', '44444444-4444-4444-4444-444444444444', 2);
insert into coupon_redemptions (code, device_id, slots)
  values ('SS7K3M2X', '44444444-4444-4444-4444-444444444444', 1);

\echo '== 6. the atomic path: the second phone takes the last use (should succeed)'
select redeem_coupon(
  'SS7K3M2X', '55555555-5555-5555-5555-555555555555',
...
insert into devices (id, platform) values ('66666666-6666-6666-6666-666666666666','ios')
  on conflict (id) do nothing;
select redeem_coupon(
  'SS7K3M2X', '66666666-6666-6666-6666-666666666666',
...
select redeem_coupon(
  'OP4HXR9B', '66666666-6666-6666-6666-666666666666',
...
select redeem_coupon(
  'ZZZZZZZZ', '66666666-6666-6666-6666-666666666666',
...
insert into coupons (code, kind, discount_percent, max_redemptions, batch, valid_from, valid_until)
  values ('SSENDED2', 'family', 100, 5, 'old ad', '2026-01-01T00:00:00Z', '2026-02-01T00:00:00Z');
select redeem_coupon(
  'SSENDED2', '66666666-6666-6666-6666-666666666666',
  'bbbbbbbb-0000-0000-0000-000000000005', 1::smallint, null,
  array['bbbbbbbb-0000-0000-0000-000000000051']::uuid[], array['s1']);

\echo '== 11. the shape 0009 made: a dash is refused, eight plain characters are taken'
-- The reason the separator went: a hyphen is behind the `?123` key on every phone keyboard, and
-- a code is typed by someone standing in an airport. The old spelling must not come back in.
insert into coupons (code, kind, discount_percent, max_redemptions, batch)
  values ('SS-W9VASX', 'family', 10, 1, 'bad');
insert into coupons (code, kind, discount_percent, max_redemptions, batch)
  values ('SSW9VASX', 'family', 10, 1, 'dashless batch');
select code from coupons where batch = 'dashless batch';
